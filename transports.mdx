...
2. For request messages using POST, the body of the HTTP request **MUST** be the "params" of the corresponding "Request" structure,
...
| tools/call               | POST  | /tools/{name}/calls |
| **Notifications**        | | |
...
| **Responses**            | | |
| elicit/request           | POST | /requests |
| ping                     | POST | /requests |
| roots/list               | POST | /requests |
| sampling/createMessage   | POST | /requests |
...
| /prompts             | GET    | prompts/list          |                             | 200: ListPromptsResult  | (3)   |
| /prompts/{name}      | POST   | prompts/get           | GetPromptRequest.params     | 200: GetPromptResult    | (1)   |
| /requests            | GET    | _multiple_            |                             | 200: ServerRequest      |       |
| /requests            | POST   | _multiple_            | ClientResult                | 202: Accepted           |       |
| /resources           | GET    | resources/list        |                             | 200: ListResourcesResult | (3)  |
| /resources           | POST   | resources/read        | ReadResourceRequest.params  | 200: ReadResourceResult  | (1)  |
...
| /tools               | GET    | tools/list            |                             | 200: ListToolsResult    | (3)   |
...
passed in an HTTP header along with the request. A cancellation request (from either client or server) should specify
the `Mcp-Request-Id` in the `requestId` of the `CancelledNotification`.
...
Any error response sent by either the server or the client **MUST** conform to the Problem Details format as defined in [RFC 9457].
Further, the response **MUST** include a "detail" property which should correspond to the "message" of the JSONRPCError schema.
The entire response **SHOULD** be mapped to the "data" property of JSONRPCError Schema.
