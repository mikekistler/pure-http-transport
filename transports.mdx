...
on endpoints specific to the type of request or notification.
...
| MCP message              | HTTP Method | HTTP Path | Request Body | Response |
| ------------------------ | ----------- | --------- | ------------ | -------- |
| **Requests**             | | | | |
| completion/complete      | POST | /completions | CompleteRequest.params | 200: CompleteResult |
| initialize               | POST | /initialize | InitializeRequest.params | 200: InitializeResult |
| logging/setLevel         | POST | /logging/setLevel | SetLevelRequest.params | 202: Accepted |
| ping                     | GET  | /ping |    | 202: Accepted |
<!-- ---------------- -->
| prompts/get              | POST | /prompts/{name} | GetPromptRequest.params | 200: GetPromptResult |
| prompts/list             | GET  | /prompts |  | 200: ListPromptsResult |
<!-- ---------------- -->
| resources/list           | GET  | /resources |  | 200: ListResourcesResult |
| resources/read           | POST | /resources | ReadResourceRequest.params | 200: ReadResourceResult |
| resources/subscribe      | POST | /resources/subscribe | SubscribeRequest.params | 202: Accepted |
| resources/templates/list | GET  | /resources/templates |   | 200: ListResourceTemplatesResult |
| resources/unsubscribe    | POST | /resources/unsubscribe | UnsubscribeRequest.params | 202: Accepted |
<!-- ---------------- -->
| tools/call               | PUT  | /tools/{name}/calls/{guid} | CallToolRequest.params | 200: CallToolResult |
| tools/list               | GET  | /tools |   | 200: ListToolsResult |
<!-- ---------------- -->
| **Notifications**        | | | | |
| notifications/cancelled          | POST | /notifications | CancelledNotification | 202: Accepted |
| notifications/initialized        | POST | /notifications | InitializedNotification | 202: Accepted |
| notifications/progress           | POST | /notifications | ProgressNotification | 202: Accepted |
| notifications/roots/list_changed | POST | /notifications | RootsListChangedNotification | 202: Accepted |
| **Responses**            | | | | |
| elicit/request           | POST | /elicit | ElicitResult | 202: Accepted |
| ping                     | POST | /ping | no body | 202: Accepted |
| roots/list               | POST | /roots | ListRootsResult | 202: Accepted |
| sampling/createMessage   | POST | /sampling | CreateMessageResult | 202: Accepted |

### Sending Messages from Server to Client

Each request or notification message sent from client to server:

1. **MUST** be the response of an HTTP GET request to the method and path specified in the table below.
2. The server **MUST** include the `MCP-Protocol-Version: <protocol-version>` header in the HTTP response to
specify the protocol version of the message and any response to the message.
2. For request messages, the body of the response **MUST** be the corresponding "Request" structure,
specified in the table below.
3. For notification messages, the body of the response **MUST** be the corresponding "Notification" structure.
4. The Content-type of the GET response for any request or notification messages **MUST** be `application/json`.

The following table provides a complete mapping from MCP message to HTTP Path for server to client messages:

| MCP message            | HTTP Path | Response | Notes |
| ---------------------- | --------- | -------- | ----- |
| **Requests**           | | | |
| elicit/request         | /requests | ElicitRequest | ID sent in response header |
| ping                   | /requests | PingRequest | ID sent in response header |
| roots/list             | /requests | ListRootsRequest | ID sent in response header |
| sampling/createMessage | /requests | CreateMessageRequest | ID sent in response header |
<!-- ---------------- -->
| **Notifications**      | | | |
| notifications/cancelled              | /notifications | CancelledNotification |  |
| notifications/message                | /notifications | LoggingMessageNotification |  |
| notifications/progress               | /notifications | ProgressNotification | |
| notifications/prompts/list_changed   | /notifications | PromptListChangedNotification |  |
| notifications/resources/list_changed | /notifications | ResourceListChangedNotification |  |
| notifications/resources/updated      | /notifications | ResourceUpdatedNotification |  |
| notifications/tools/list_changed     | /notifications | ToolsListChangedNotification | |

The server **MUST** implement all the endpoints specified in this table.
